...
import (
	...
	"github.com/gedwolmen/heretic/internal/backend"
	"github.com/gedwolmen/heretic/internal/config"
	_ "github.com/gedwolmen/heretic/internal/swagger"
...
// socketDir returns the directory used for the Heretic Unix socket.
...
// returned by [socketDir] and is named heretic-<uid>.sock, falling
// back to heretic.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/heretic-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "heretic.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("heretic-%s.sock", usr.Uid)
	}
...
// Server represents a Heretic server bound to a specific address.
type Server struct {
