...
// are what non-interactive clients (e.g. `heretic run`) rely on to
...
// through the SSE envelope. Without these correlators, `heretic run`
...
// "agent failed" (returns non-zero from `heretic run`) from "agent
