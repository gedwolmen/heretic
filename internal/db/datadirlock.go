...
// already in use by another heretic process.
var ErrDataDirLocked = errors.New("data directory already in use by another heretic process")

// dataDirLockFile is the name of the lock file inside the data
// directory. It lives next to heretic.db so users can `ls` and find it.
const dataDirLockFile = "heretic.lock"
...
// {dataDir}/heretic.lock. If the lock is already held by another
...
// HERETIC_SKIP_DATADIR_LOCK is set to a truthy value. This is intended
...
func skipDataDirLock() bool {
	v, _ := strconv.ParseBool(os.Getenv("HERETIC_SKIP_DATADIR_LOCK"))
	return v
}
