...
// heretic process from racing the storage.
...
// workspace-bootstrap path opts in. HERETIC_SKIP_DATADIR_LOCK still
...
func Connect(ctx context.Context, dataDir string, opts ...ConnectOption) (*sql.DB, error) {
	...
	dbPath := filepath.Join(dataDir, "heretic.db")
	...
	// heretic process on the same SQLite file. The lock is released when
	...
	// local-mode invocations do not refuse a second heretic against the
