...
func TestToPromptXMLBuiltinType(t *testing.T) {
	...
	skills := []*Skill{
		{Name: "builtin-skill", Description: "A builtin.", SkillFilePath: "heretic://skills/builtin-skill/SKILL.md", Builtin: true},
		{Name: "user-skill", Description: "A user skill.", SkillFilePath: "/home/user/.config/heretic/skills/user-skill/SKILL.md"},
	}
	xml := ToPromptXML(skills)
...
func TestDiscoverBuiltin(t *testing.T) {
	...
	for _, s := range discovered {
		if s.Name == "heretic-config" {
			found = true
			require.True(t, strings.HasPrefix(s.SkillFilePath, BuiltinPrefix))
			require.True(t, strings.HasPrefix(s.Path, BuiltinPrefix))
			require.Equal(t, "heretic://skills/heretic-config/SKILL.md", s.SkillFilePath)
			require.Equal(t, "heretic://skills/heretic-config", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, found, "heretic-config builtin skill not found")

	var foundJQ bool
	for _, s := range discovered {
		if s.Name == "jq" {
			foundJQ = true
			require.Equal(t, "heretic://skills/jq/SKILL.md", s.SkillFilePath)
			require.Equal(t, "heretic://skills/jq", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	...
	var foundHooks bool
	for _, s := range discovered {
		if s.Name == "heretic-hooks" {
			foundHooks = true
			require.Equal(t, "heretic://skills/heretic-hooks/SKILL.md", s.SkillFilePath)
			require.Equal(t, "heretic://skills/heretic-hooks", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, foundHooks, "heretic-hooks builtin skill not found")
}
...
func TestDeduplicate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    []*Skill{{Name: "heretic-config", Path: "heretic://skills/heretic-config"}, {Name: "heretic-config", Path: "/user/heretic-config"}},
			wantLen:  1,
			wantName: "heretic-config",
			wantPath: "/user/heretic-config",
		},
		{
			name:    "empty",
			input:   nil,
