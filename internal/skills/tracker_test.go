...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (heretic-config).
	activeSkills := []*Skill{
		{Name: "heretic-config", Description: "Heretic config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("heretic-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via heretic://...).
	tracker.MarkLoaded("heretic-config")
	require.True(t, tracker.IsLoaded("heretic-config"))
