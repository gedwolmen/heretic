---
name: heretic-config
description: Use when the user needs help configuring Heretic — working with heretic.json, setting up providers, configuring LSPs, adding MCP servers, managing skills or permissions, or changing Heretic behavior.
---

# Heretic Configuration

Heretic uses JSON configuration files with the following priority (highest to lowest):

1. `.heretic.json` (project-local, hidden)
2. `heretic.json` (project-local)
3. `$XDG_CONFIG_HOME/heretic/heretic.json` or `$HOME/.config/heretic/heretic.json` (global)

## Basic Structure

```json
{
  "$schema": "https://raw.githubusercontent.com/gedwolmen/heretic/main/heretic.json.json",
  "models": {},
  "providers": {},
  "mcp": {},
  "lsp": {},
...
Heretic runs selected string fields through an embedded bash-compatible
...
`heretic.json` is trusted code. Any `$(...)` in it runs at load time
with the invoking user's shell privileges, before the UI appears.
Don't launch Heretic in a directory whose `heretic.json` you haven't
reviewed.
...
```json
{
  "options": {
    ...
    "disabled_skills": ["heretic-config"],
    "tui": {
      "compact_mode": false,
      "diff_mode": "unified",
      "transparent": false
...
> `.agents/skills`, `.heretic/skills`, `.claude/skills`, `.cursor/skills`
    ...
    "PreToolUse": [
      {
        ...
        "command": ".heretic/hooks/protect-files.sh"
      },
      {
        "matcher": "^bash$",
        "command": ".heretic/hooks/no-haskell.sh"
      }
    ]
  }
}
...
|---|---|
| `HERETIC_EVENT` | Event name (e.g. `PreToolUse`) |
| `HERETIC_TOOL_NAME` | Name of the tool being called |
| `HERETIC_SESSION_ID` | Current session ID |
| `HERETIC_CWD` | Current working directory |
| `HERETIC_PROJECT_DIR` | Project root directory |
| `HERETIC_TOOL_INPUT_COMMAND` | Value of `command` from tool input (if present) |
| `HERETIC_TOOL_INPUT_FILE_PATH` | Value of `file_path` from tool input (if present) |
...
Heretic also supports the Claude Code hook output format:
...
- `HERETIC_GLOBAL_CONFIG` - Override global config location
- `HERETIC_GLOBAL_DATA` - Override data directory location
- `HERETIC_SKILLS_DIR` - Override default skills directory
