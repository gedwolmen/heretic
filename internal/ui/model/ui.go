...
import (
	...
	"github.com/gedwolmen/heretic/internal/agent/hyper"
	"github.com/gedwolmen/heretic/internal/agent/notify"
	agenttools "github.com/gedwolmen/heretic/internal/agent/tools"
	"github.com/gedwolmen/heretic/internal/agent/tools/mcp"
	"github.com/gedwolmen/heretic/internal/app"
	"github.com/gedwolmen/heretic/internal/clipboard"
	"github.com/gedwolmen/heretic/internal/commands"
	commandsbuiltin "github.com/gedwolmen/heretic/internal/commands/builtin"
	"github.com/gedwolmen/heretic/internal/config"
	...
	"github.com/gedwolmen/heretic/internal/session"
	"github.com/gedwolmen/heretic/internal/skills"
	"github.com/gedwolmen/heretic/internal/stringext"
	"github.com/gedwolmen/heretic/internal/ui/anim"
	"github.com/gedwolmen/heretic/internal/ui/attachments"
	"github.com/gedwolmen/heretic/internal/ui/chat"
	"github.com/gedwolmen/heretic/internal/ui/common"
	"github.com/gedwolmen/heretic/internal/ui/completions"
	"github.com/gedwolmen/heretic/internal/ui/dialog"
	fimage "github.com/gedwolmen/heretic/internal/ui/image"
	"github.com/gedwolmen/heretic/internal/ui/logo"
	"github.com/gedwolmen/heretic/internal/ui/notification"
	"github.com/gedwolmen/heretic/internal/ui/styles"
	"github.com/gedwolmen/heretic/internal/ui/util"
	"github.com/gedwolmen/heretic/internal/version"
	"github.com/gedwolmen/heretic/internal/workspace"
)
...
func (m *UI) loadCustomCommands() tea.Cmd {
	return func() tea.Msg {
		...
		// Append the builtin OmO-style commands (ralph-loop, refactor,
		// start-work, etc.). These are embedded in the binary and
		// always available regardless of workspace state.
		if builtins, err := commandsbuiltin.Load(); err == nil {
			customCommands = append(customCommands, builtins...)
		} else {
			slog.Error("Failed to load builtin commands", "error", err)
		}
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case app.UpdateAvailableMsg:
		text := fmt.Sprintf("Heretic update available: v%s → v%s.", msg.CurrentVersion, msg.LatestVersion)
		if msg.IsDevelopment {
			text = fmt.Sprintf("This is a development version of Heretic. The latest version is v%s.", msg.LatestVersion)
		}
		ttl := 10 * time.Second
...
func (m *UI) handleKeyPressMsg(msg tea.KeyPressMsg) tea.Cmd {
	...
	handleGlobalKeys := func(msg tea.KeyPressMsg) bool {
		...
		case key.Matches(msg, m.keyMap.AgentSwitch):
			if cmd := m.openAgentSwitchDialog(); cmd != nil {
				cmds = append(cmds, cmd)
			}
			return true
...
func (m *UI) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	if os.Getenv("HERETIC_UI_DEBUG") == "true" {
...
func (m *UI) View() tea.View {
	...
	v.WindowTitle = "heretic " + home.Short(m.com.Workspace.WorkingDir())
...
// currentAgentName returns the registry name of the agent that
// the current session is using. Falls back to sisyphus.
func (m *UI) currentAgentName() dialog.AgentName {
	if m.currentAgent == "" {
		return dialog.AgentSisyphus
	}
	switch m.currentAgent {
	case dialog.AgentSisyphus, dialog.AgentHephaestus, dialog.AgentAtlas, dialog.AgentPrometheus:
		return m.currentAgent
	}
	return dialog.AgentSisyphus
}

// setAgent updates the current agent on the UI and (when possible)
// on the active session. The preference is recorded for the next
// session start. Mid-session hot-swap is out of scope (would require
// rebuilding the agent's tools + prompt); the user sees a status
// message and a header pill confirming the new agent.
func (m *UI) setAgent(name dialog.AgentName) {
	m.currentAgent = name
	if m.session != nil {
		m.session.AgentName = string(name)
	}
}

// openAgentSwitchDialog opens the agent switcher dialog.
func (m *UI) openAgentSwitchDialog() tea.Cmd {
	if m.dialog.ContainsDialog(dialog.AgentSwitchID) {
		m.dialog.BringToFront(dialog.AgentSwitchID)
		return nil
	}
	dlg := dialog.NewAgentSwitch(m.com, m.currentAgentName())
