...
import (
	...
	"github.com/gedwolmen/heretic/internal/ui/list"
...
func NewCommands(com *common.Common, sessionID string, hasSession, hasTodos, hasQueue bool, customCommands []commands.CustomCommand, mcpPrompts []commands.MCPPrompt) (*Commands, error) {
	...
	c.input.Placeholder = "Type a command, e.g. /ralph-loop"
...
func (c *Commands) HandleMsg(msg tea.Msg) Action {
	...
	case tea.KeyPressMsg:
		...
		default:
			...
			value := c.input.Value()
			// Strip leading "/" so "/ralph-loop" matches "ralph-loop".
...
func (c *Commands) setCommandItems(commandType CommandType) {
	...
	case UserCommands:
		for _, cmd := range c.customCommands {
			...
			// shown as a trailing subtitle instead.
			name := cmd.Name
			desc := ""
			if cmd.Builtin {
				name = "/" + name
				desc = "(builtin)"
				if i := strings.Index(cmd.Content, "\n"); i > 0 {
					first := strings.TrimSpace(cmd.Content[:i])
					if len(first) > 80 {
						first = first[:77] + "..."
					}
					desc = first + "  (" + desc + ")"
			...
			if cmd.Skill != nil {
				...
				item = item.WithDescription(desc)
			}
