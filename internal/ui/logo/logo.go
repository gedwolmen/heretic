...
func Render(base lipgloss.Style, version string, compact bool, o Opts) string {
	...
	if o.Hyper && !compact {
		hereticLetterforms = append(hyperLetterforms, hereticLetterforms...)
	}

	stretchIndex := -1 // -1 means no stretching.
	if !compact && !o.Unstable {
		// Always stretch the same letterform, which is picked once at random.
		stretchIndex = cachedRandN(len(hereticLetterforms))
	} else if !compact && o.Unstable {
		// Stretch a random letterform on every render.
		stretchIndex = rand.IntN(len(hereticLetterforms))
	}
	heretic := renderWord(spacing, stretchIndex, hereticLetterforms...)
	if o.Hyper && compact {
		heretic = renderWord(spacing, stretchIndex, hyperLetterforms...) + "\n" + heretic
	}
