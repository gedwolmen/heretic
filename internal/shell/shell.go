...
// HereticEnvMarkers returns a fresh slice of the environment variables that
// Heretic unconditionally sets on every shell it spawns — both the interactive
...
func NewShell(opts *Options) *Shell {
	...
	// Allow tools to detect execution by Heretic.
	env = append(env, HereticEnvMarkers()...)
