...
type RunOptions struct {
	...
	// the Heretic process cwd — hooks and the bash tool have different
...
// Heretic handler stack. Shared by the stateless [Run] entrypoint and the
...
// standard Heretic middleware chain (builtins, script dispatch, block list)
...
// when sourcing framework files can send SIGINT/SIGTERM to Heretic's process
...
// regardless of the caller's environment because Heretic shells are never
...
//  1. builtins first (so Heretic's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized Heretic
