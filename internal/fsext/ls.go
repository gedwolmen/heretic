...
// hereticGlobalIgnorePatterns returns patterns from the user's
// ~/.config/heretic/ignore file.
var hereticGlobalIgnorePatterns = sync.OnceValue(func() []gitignore.Pattern {
	name := filepath.Join(home.Config(), "heretic", "ignore")
	bts, err := os.ReadFile(name)
	if err != nil {
		if !os.IsNotExist(err) {
			slog.Debug("Failed to read heretic global ignore file", "path", name, "error", err)
		}
		return nil
	}
...
type directoryLister struct {
	// dirPatterns caches parsed patterns from .gitignore/.hereticignore for each directory.
...
// .gitignore and .hereticignore files. Results are cached.
func (dl *directoryLister) getDirPatterns(dir string) []gitignore.Pattern {
	return dl.dirPatterns.GetOrSet(dir, func() []gitignore.Pattern {
		...
		for _, ignoreFile := range []string{".gitignore", ".hereticignore"} {
...
func (dl *directoryLister) getCombinedMatcher(dir string) gitignore.Matcher {
	return dl.combinedMatchers.GetOrSet(dir, func() gitignore.Matcher {
		...
		// Add global ignore patterns (git core.excludesFile + heretic global ignore).
		allPatterns = append(allPatterns, gitGlobalIgnorePatterns()...)
		allPatterns = append(allPatterns, hereticGlobalIgnorePatterns()...)
