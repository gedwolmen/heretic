...
func TestHereticIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .hereticignore file that ignores .log files
	require.NoError(t, os.WriteFile(".hereticignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .hereticignore file
	hereticignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".hereticignore"), []byte(hereticignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .hereticignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .hereticignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .hereticignore in subdir that ignores normal_nested
	subHereticignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".hereticignore"), []byte(subHereticignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .hereticignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .hereticignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .hereticignore")
