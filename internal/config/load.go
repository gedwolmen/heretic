...
import (
	...
	"github.com/gedwolmen/heretic/internal/filepathext"
...
func PushPopHereticEnv() func() {
	var found []string
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "HERETIC_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "HERETIC_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("HERETIC_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if len(c.Options.GlobalContextPaths) == 0 {
		hereticConfigDir := filepath.Dir(GlobalConfig())
		c.Options.GlobalContextPaths = []string{
			filepath.Join(hereticConfigDir, "HERETIC.md"),
			filepath.Join(filepath.Dir(hereticConfigDir), "AGENTS.md"),
		}
	}
	...
	if str, ok := os.LookupEnv("HERETIC_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("HERETIC_DISABLE_DEFAULT_PROVIDERS"); ok {
...
// so an unrelated heretic.json placed above the project is never picked
...
func GlobalConfig() string {
	if hereticGlobal := os.Getenv("HERETIC_GLOBAL_CONFIG"); hereticGlobal != "" {
		return filepath.Join(hereticGlobal, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalCacheDir() string {
	if hereticCache := os.Getenv("HERETIC_CACHE_DIR"); hereticCache != "" {
		return hereticCache
	}
...
func GlobalConfigData() string {
	if hereticData := os.Getenv("HERETIC_GLOBAL_DATA"); hereticData != "" {
		return filepath.Join(hereticData, fmt.Sprintf("%s.json", appName))
	}
	...
	// for windows, it should be in `%LOCALAPPDATA%/heretic/`
	// for linux and macOS, it should be in `$HOME/.local/share/heretic/`
	if runtime.GOOS == "windows" {
		localAppData := cmp.Or(
			os.Getenv("LOCALAPPDATA"),
			filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local"),
...
// fallback keeps Heretic from silently adopting state files placed above
// the current project.
...
func GlobalSkillsDirs() []string {
	if hereticSkills := os.Getenv("HERETIC_SKILLS_DIR"); hereticSkills != "" {
		return []string{hereticSkills}
	}
	...
	// On Windows, also load from app data on top of `$HOME/.config/heretic`.
...
// ProjectSkillsDir returns the default project directories for which Heretic
