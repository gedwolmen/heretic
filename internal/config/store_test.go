...
func TestConfigStore_ConfigPath_GlobalAlwaysWorks(t *testing.T) {
	...
	store := &ConfigStore{
		globalDataPath: "/some/global/heretic.json",
	}

	path, err := store.configPath(ScopeGlobal)
	require.NoError(t, err)
	require.Equal(t, "/some/global/heretic.json", path)
}
...
func TestConfigStore_ConfigPath_WorkspaceReturnsPath(t *testing.T) {
	...
	store := &ConfigStore{
		workspacePath: "/some/workspace/.heretic/heretic.json",
	}

	path, err := store.configPath(ScopeWorkspace)
	require.NoError(t, err)
	require.Equal(t, "/some/workspace/.heretic/heretic.json", path)
}
...
func TestGlobalWorkspaceDir(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("HERETIC_GLOBAL_DATA", dir)
...
func TestReloadFromDisk_UsesNewConfigValues(t *testing.T) {
	...
	t.Setenv("HERETIC_GLOBAL_CONFIG", dir)
	t.Setenv("HERETIC_GLOBAL_DATA", dir)
	resetProviderState()
