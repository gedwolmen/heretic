...
import (
	...
	"github.com/gedwolmen/heretic/internal/csync"
	"github.com/gedwolmen/heretic/internal/oauth"
	"github.com/gedwolmen/heretic/internal/oauth/copilot"
	"github.com/invopop/jsonschema"
)

const (
	appName              = "heretic"
	defaultDataDirectory = ".heretic"
...
type ProviderConfig struct {
	...
	// When Models is empty and this is nil or true, Heretic auto-discovers
...
type Attribution struct {
	...
	GeneratedWith bool         `json:"generated_with,omitempty" jsonschema:"description=Add Generated with Heretic line to commit messages and issues and PRs,default=true"`
}
...
type Options struct {
	ContextPaths         []string    `json:"context_paths,omitempty" jsonschema:"description=Paths to files containing context information for the AI,example=.cursorrules,example=HERETIC.md"`
	GlobalContextPaths   []string    `json:"global_context_paths,omitempty" jsonschema:"description=Paths to files containing global context information for the AI,default=~/.config/heretic/HERETIC.md,default=~/.config/AGENTS.md"`
	SkillsPaths          []string    `json:"skills_paths,omitempty" jsonschema:"description=Paths to directories containing Agent Skills (folders with SKILL.md files),example=~/.config/heretic/skills,example=./skills"`
	TUI                  *TUIOptions `json:"tui,omitempty" jsonschema:"description=Terminal user interface options"`
	Debug                bool        `json:"debug,omitempty" jsonschema:"description=Enable debug logging,default=false"`
	DebugLSP             bool        `json:"debug_lsp,omitempty" jsonschema:"description=Enable debug logging for LSP servers,default=false"`
	DisableAutoSummarize bool        `json:"disable_auto_summarize,omitempty" jsonschema:"description=Disable automatic conversation summarization,default=false"`
	// DataDirectory is where Heretic keeps per-project state such as
	// the SQLite database and workspace overrides. Relative paths are
	// resolved against the working directory; absolute paths are used
	// verbatim. After defaulting the stored value is always absolute.
	DataDirectory             string       `json:"data_directory,omitempty" jsonschema:"description=Directory for storing application data. Relative paths are resolved against the working directory; absolute paths are used as-is.,default=.heretic,example=.heretic"`
	...
	InitializeAs              string       `json:"initialize_as,omitempty" jsonschema:"description=Name of the context file to create/update during project initialization,default=AGENTS.md,example=AGENTS.md,example=HERETIC.md,example=CLAUDE.md,example=docs/LLMs.md"`
	AutoLSP                   *bool        `json:"auto_lsp,omitempty" jsonschema:"description=Automatically setup LSPs based on root markers,default=true"`
	AutoDetectMCP             *bool        `json:"auto_detect_mcp,omitempty" jsonschema:"description=Automatically discover MCP servers from Claude Desktop\\, Cursor\\, and Claude Code (.mcp.json) configs on the host and merge them at startup. Servers already declared in the heretic config are not overwritten.,default=true"`
	...
	DisabledSkills            []string     `json:"disabled_skills,omitempty" jsonschema:"description=List of skill names to disable and hide from the agent,example=heretic-config"`
}
...
// Config holds the configuration for heretic.
type Config struct {
	...
	// TeamMode configures parallel multi-agent coordination.
	TeamMode TeamModeConfig `json:"team_mode,omitempty" jsonschema:"description=Team mode (parallel multi-agent coordination)"`

	// BackgroundAgent configures background agent concurrency.
	BackgroundAgent BackgroundAgentConfig `json:"background_agent,omitempty" jsonschema:"description=Background agent concurrency limits"`

	// BoulderState configures the boulder feature.
	BoulderState BoulderStateConfig `json:"boulder_state,omitempty" jsonschema:"description=Boulder state (work tracking) configuration"`

	// Delegate configures the task tool's concurrency.
	Delegate DelegateConfig `json:"delegate,omitempty" jsonschema:"description=Subagent delegation settings"`
