...
// with reason=tool_use and used to terminate `heretic run` early via
...
// subscribers (e.g. a TUI watching workspace events while `heretic
...
// non-nil error from `heretic run` so shells and CI catch it via
// exit status.
...
// run (e.g. Ctrl+C while `heretic run` waits) exits cleanly rather
...
// queue scenario: `heretic run --continue` attaches to a session
