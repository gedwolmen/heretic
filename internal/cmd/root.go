...
import (
	...
	"github.com/gedwolmen/heretic/internal/client"
	"github.com/gedwolmen/heretic/internal/config"
	"github.com/gedwolmen/heretic/internal/db"
	"github.com/gedwolmen/heretic/internal/event"
	"github.com/gedwolmen/heretic/internal/lock"
	hereticlog "github.com/gedwolmen/heretic/internal/log"
	"github.com/gedwolmen/heretic/internal/projects"
	"github.com/gedwolmen/heretic/internal/proto"
	"github.com/gedwolmen/heretic/internal/server"
	...
	ui "github.com/gedwolmen/heretic/internal/ui/model"
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&clientHost, "host", "H", server.DefaultHost(), "Connect to a specific heretic server host (for advanced users)")
...
heretic run "Guess my 5 favorite Pokémon"

# Run a non-interactively with pipes and redirection
cat README.md | heretic run "make this more glamorous" > GLAMOROUS_README.md

# Run with debug logging in a specific directory
heretic --debug --cwd /path/to/project

# Run in yolo mode (auto-accept all permissions; use with care)
heretic --yolo

# Run with custom data directory
heretic --data-dir /path/to/custom/.heretic

# Continue a previous session
heretic --session {session-id}
  ...
  `,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, err := program.Run(); err != nil {
			...
			return errors.New("Heretic crashed. If metrics are enabled, we were notified about it. If you'd like to report it, please copy the stacktrace above and open an issue at https://github.com/gedwolmen/heretic/issues/new?template=bug.yml") //nolint:staticcheck
		}
		return nil
	},
}
...
// enabled via the HERETIC_CLIENT_SERVER environment variable.
func useClientServer() bool {
	v, _ := strconv.ParseBool(os.Getenv("HERETIC_CLIENT_SERVER"))
	return v
}
...
// HERETIC_CLIENT_SERVER=1, it connects to a server process and returns a
...
func setupLocalWorkspace(cmd *cobra.Command) (workspace.Workspace, func(), error) {
	...
	logFile := filepath.Join(cfg.Options.DataDirectory, "logs", "heretic.log")
	hereticlog.Setup(logFile, debug)
...
func connectToServer(cmd *cobra.Command) (*client.Client, *proto.Workspace, func(), error) {
	...
	if ws.Config != nil {
		logFile := filepath.Join(ws.Config.Options.DataDirectory, "logs", "heretic.log")
		hereticlog.Setup(logFile, debug)
	}
...
func ensureServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	// server log file. hereticlog.Setup uses sync.Once internally, so the
	// later call from connectToServer becomes a no-op.
	debug, _ := cmd.Flags().GetBool("debug")
	logFile := filepath.Join(config.GlobalCacheDir(), "server-"+safeHostName(hostURL), "heretic.log")
	hereticlog.Setup(logFile, debug)
	...
	case "unix", "npipe":
		...
		if needsStart {
			if err := spawnAndWaitReady(cmd, hostURL); err != nil {
				return fmt.Errorf("failed to initialize heretic server: %v", err)
			}
			return nil
		}
...
// $XDG_CACHE_HOME/heretic/server-<safeHost>/start.lock.
//
...
// Overridable via HERETIC_SERVER_READY_TIMEOUT (parsed as a Go duration).
func serverReadyTimeout() time.Duration {
	const def = 10 * time.Second
	v := os.Getenv("HERETIC_SERVER_READY_TIMEOUT")
	if v == "" {
		return def
	}
...
func startDetachedServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	if err := c.Start(); err != nil {
		return fmt.Errorf("failed to start heretic server: %v", err)
	}

	if err := c.Process.Release(); err != nil {
		return fmt.Errorf("failed to detach heretic server process: %v", err)
	}
...
func shouldEnableMetrics(cfg *config.Config) bool {
	if v, _ := strconv.ParseBool(os.Getenv("HERETIC_DISABLE_METRICS")); v {
		return false
	}
...
func createDotHereticDir(dir string) error {
