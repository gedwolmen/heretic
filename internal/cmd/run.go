...
import (
	...
	"github.com/gedwolmen/heretic/internal/format"
...
curl https://charm.land | heretic run "Summarize this website"

# Read from a file
heretic run "What is this code doing?" <<< prrr.go

# Redirect output to a file
heretic run "Generate a hot README for this project" > MY_HOT_README.md

# Run in quiet mode (hide the spinner)
heretic run --quiet "Generate a README for this project"

# Run in verbose mode (show logs)
heretic run --verbose "Generate a README for this project"

# Continue a previous session
heretic run --session {session-id} "Follow up on your last response"

# Continue the most recent session
heretic run --continue "Follow up on your last response"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if useClientServer() {
			...
			if !ws.Config.IsConfigured() {
				return fmt.Errorf("no providers configured - please run 'heretic' to set up a provider interactively")
			}
...
// streaming SSE events into a final, complete stdout for `heretic run`.
...
func (s *runStream) handle(ev any, stopSpinner func()) (done bool, err error) {
	...
	case pubsub.Event[proto.RunComplete]:
		...
		// source of the regression where `heretic run` exited
...
// prefix returned by heretic session list.
