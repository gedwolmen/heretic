...
import (
	...
	"github.com/gedwolmen/heretic/internal/agent/prompt"
	"github.com/gedwolmen/heretic/internal/agent/tools"
	"github.com/gedwolmen/heretic/internal/config"
	"github.com/gedwolmen/heretic/internal/discover"
	"github.com/gedwolmen/heretic/internal/event"
	"github.com/gedwolmen/heretic/internal/filetracker"
	"github.com/gedwolmen/heretic/internal/history"
	"github.com/gedwolmen/heretic/internal/hooks"
	"github.com/gedwolmen/heretic/internal/log"
...
type coordinator struct {
	...
	// preferredAgent is the registry name of the agent the user picked
	// in the in-TUI agent switcher. Empty means default (coder agent).
	// New sessions honor this preference; the current session keeps
	// using the agent it started with (full mid-session swap is out of
	// scope for the in-TUI switcher).
...
func (c *coordinator) run(ctx context.Context, accept *AcceptedRun, sessionID string, prompt string, attachments ...message.Attachment) (*fantasy.AgentResult, error) {
	...
	// retry's success, and `heretic run` would exit on the stale error
...
func (c *coordinator) buildTools(ctx context.Context, agent config.Agent, isSubAgent bool) ([]fantasy.AgentTool, error) {
	...
	logFile := filepath.Join(c.cfg.Config().Options.DataDirectory, "logs", "heretic.log")
	...
	allTools = append(
		...
		tools.NewHereticInfoTool(c.cfg, c.lspManager, c.allSkills, c.activeSkills, c.skillTracker),
		tools.NewHereticLogsTool(logFile),
...
func (c *coordinator) buildProvider(providerCfg config.ProviderConfig, model config.SelectedModel, isSubAgent bool) (fantasy.Provider, error) {
	...
	case openaicompat.Name, hyper.Name:
		...
		case hyper.Name:
			...
			headers["x-heretic-id"] = event.GetID()
...
// CurrentAgent returns the SessionAgent currently in use.
func (c *coordinator) CurrentAgent() SessionAgent {
	return c.currentAgent
}

// SetCurrentAgentName records which main agent the user picked via
// the in-TUI switcher dialog. The current session keeps using the
// agent it started with; the change takes effect on the next session.
func (c *coordinator) SetCurrentAgentName(name string) {
	c.preferredAgent = name
}

// PreferredAgentName returns the agent name the user picked via the
// switcher. Empty if the user has not picked one.
func (c *coordinator) PreferredAgentName() string {
	return c.preferredAgent
}
