// Package agent is the core orchestration layer for Heretic AI agents.
//
...
var userAgent = fmt.Sprintf("Charm-Heretic/%s (https://charm.land/heretic)", version.Version)
...
type SessionAgentCall struct {
	...
	// reliable completion contract (e.g. `heretic run` against a
	...
	// event, so non-interactive clients (e.g. `heretic run`) don't
...
// RunID, e.g. `heretic run`, would otherwise hang). Uncanceled calls without
...
// that RunID: `heretic run` ignores live message events and exits only on a
...
// so callers waiting on those RunIDs (e.g. `heretic run`) are not left
...
// caller waiting on RunComplete (e.g. `heretic run` with a RunID) always
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if call.Accepted != nil {
		...
		if a.canceledBySeq(call.SessionID, call.Accepted.seq) {
			...
			// RunComplete for this RunID (e.g. `heretic run`, which
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "heretic auth" to re-authenticate.`)
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("HERETIC_DISABLE_ANTHROPIC_CACHE")); t {
