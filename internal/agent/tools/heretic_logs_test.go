...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "heretic.log")
...
func TestNewHereticLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewHereticLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, HereticLogsToolName, tool.Info().Name)
}

func TestHereticLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runHereticLogs(logFile, HereticLogsParams{Lines: 3})
...
func TestHereticLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runHereticLogs(logFile, HereticLogsParams{Lines: 0})
...
func TestHereticLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runHereticLogs(logFile, HereticLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestHereticLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runHereticLogs("/nonexistent/path/heretic.log", HereticLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestHereticLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "heretic.log")
	...
	result := runHereticLogs(logFile, HereticLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestHereticLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "heretic.log")
	...
	result := runHereticLogs(logFile, HereticLogsParams{Lines: 10})
...
func TestHereticLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runHereticLogs(logFile, HereticLogsParams{Lines: 1})
...
func TestHereticLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestHereticLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestHereticLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestHereticLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "heretic.log")
...
func TestHereticLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "heretic.log")
...
func TestHereticLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestHereticLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestHereticLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestHereticLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runHereticLogs(logFile, HereticLogsParams{Lines: 5})
...
func TestHereticLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
