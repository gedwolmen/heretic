...
func TestReadBuiltinFile(t *testing.T) {
	...
	t.Run("reads heretic-config skill", func(t *testing.T) {
		t.Parallel()

		resp, err := readBuiltinFile(ViewParams{
			FilePath: "heretic://skills/heretic-config/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.NotEmpty(t, resp.Content)
		require.Contains(t, resp.Content, "Heretic Configuration")
	})
	...
	t.Run("not found", func(t *testing.T) {
		...
		resp, err := readBuiltinFile(ViewParams{
			FilePath: "heretic://skills/nonexistent/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.True(t, resp.IsError)
	})
	...
	t.Run("metadata has skill info", func(t *testing.T) {
		...
		require.Equal(t, "heretic-config", meta.ResourceName)
