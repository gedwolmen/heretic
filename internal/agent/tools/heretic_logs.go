...
const HereticLogsToolName = "heretic_logs"

//go:embed heretic_logs.md.tpl
var hereticLogsDescriptionTmpl []byte

var hereticLogsDescriptionTpl = template.Must(
	template.New("hereticLogsDescription").
		Parse(string(hereticLogsDescriptionTmpl)),
)

type hereticLogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func hereticLogsDescription() string {
	return renderTemplate(hereticLogsDescriptionTpl, hereticLogsDescriptionData{
...
type HereticLogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewHereticLogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		HereticLogsToolName,
		hereticLogsDescription(),
		func(ctx context.Context, params HereticLogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runHereticLogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runHereticLogs reads and formats the last N log entries from the given file.
func runHereticLogs(logFile string, params HereticLogsParams) string {
