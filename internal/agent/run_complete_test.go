...
// event into a closure nobody reads, and subscribers (`heretic run`)
...
// publish a RunComplete for its RunID, hanging a `heretic run` caller that
...
// than internal bookkeeping, which is the contract a `heretic run` caller
...
// bookkeeping slice, not the published event a `heretic run` caller awaits.
