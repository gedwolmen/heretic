...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["HERETIC_EVENT"])
	require.Equal(t, "bash", envMap["HERETIC_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["HERETIC_SESSION_ID"])
	require.Equal(t, "/work", envMap["HERETIC_CWD"])
	require.Equal(t, "/project", envMap["HERETIC_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["HERETIC_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["HERETIC_TOOL_INPUT_FILE_PATH"])

	// Shared heretic markers must be present so hook-authored scripts can
	// detect they're running under heretic the same way bash-tool-invoked
	// scripts can.
	require.Equal(t, "1", envMap["HERETIC"])
	require.Equal(t, "heretic", envMap["AGENT"])
	require.Equal(t, "heretic", envMap["AI_AGENT"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$HERETIC_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("heretic format still works", func(t *testing.T) {
		t.Parallel()
