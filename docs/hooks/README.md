...
the agent lifecycle, allowing you to both build on top of Heretic, customize
...
- Heretic ships with a builtin `heretic-hook` skill write, edit, and configure
  hooks; just tell Heretic how to configure Heretic
- Heretic currently supports just one hook, `PreToolUse`, with plans to support
...
our **project-level** `heretic.json`. Relative paths like `./no-haskell.sh` work
here because the project root is your working directory. If you're configuring
a global hook (`~/.config/heretic/heretic.json`), use an absolute path instead.
...
# ($HERETIC_TOOL_INPUT_COMMAND) to grep and match on a regexp.
if echo "$HERETIC_TOOL_INPUT_COMMAND" | grep -qE '(^| )((ghc|cabal|stack)(\.exe)?)( |$)'; then
...
Hooks run through Heretic's embedded POSIX shell (`mvdan.cc/sh`) — the same
...
- **Permissive shebang fallback**: if the absolute path in a shebang
  doesn't exist (e.g. `#!/bin/bash` on Windows), Heretic falls back to a
  ...
  "no opinion".
- **Environment**: every hook sees `HERETIC=1`, `AGENT=heretic`, and
  `AI_AGENT=heretic` on top of the `HERETIC_*` hook-specific variables. These
  ...
  both contexts.
- **Timeout behavior**: when a hook exceeds its timeout, Heretic cancels the
  context and waits a short grace period (~1s) for the interpreter to
  yield. If the hook still hasn't returned, Heretic abandons it, logs a
...
Hooks can be added to your `heretic.json` (or `.heretic.json`) at both the global
...
> work fine in project-level `heretic.json` because the project root is also
> your working directory. For **global** config (`~/.config/heretic/`),
> however, you must use either an absolute path or an inline command:
>
> ```jsonc
> // Global ~/.config/heretic/heretic.json
> {
>   "hooks": {
>     "PreToolUse": [
>       {
>         "command": "/home/you/.config/heretic/hooks/no-haskell.sh"
...
3. Runs all matching hooks **in parallel** through Heretic's embedded POSIX
...
5. Applies the result **before** permission checks. If the aggregated decision
   ...
   for it. If it's `allow`, Heretic treats that as affirmative pre-approval and
...
however you'll incur some additional overhead as Heretic will still parse and
run each hook.
...
| ---------------------------- | ---------------------------------------------- |
| HERETIC                      | Always `1` when running under Heretic.           |
| `AGENT`                      | Always `heretic`.                              |
| `AI_AGENT`                   | Always `heretic`.                              |
| `HERETIC_EVENT`                | The hook event name (e.g. `PreToolUse`).       |
| `HERETIC_TOOL_NAME`            | The tool being called (e.g. `bash`).           |
| `HERETIC_SESSION_ID`           | Current session ID.                            |
| `HERETIC_CWD`                  | Working directory.                             |
| `HERETIC_PROJECT_DIR`          | Project root directory.                        |
| `HERETIC_TOOL_INPUT_COMMAND`   | For `bash` calls: the shell command being run. |
| `HERETIC_TOOL_INPUT_FILE_PATH` | For file tools: the target file path.          |

The HERETIC, `AGENT`, and `AI_AGENT` markers are also set by the `bash`
tool, so a script can detect "am I running under Heretic?" the same way in
either context.
...
Hooks communicate back to Heretic via **exit code** and `stdout`/`stderr`. The
...
deterministic based on the order hooks appear in `heretic.json`.
...
If a hook exceeds its timeout, Heretic cancels its context and treats the
...
if echo "$HERETIC_TOOL_INPUT_COMMAND" | grep -qE 'rm\s+-(rf|fr)\s+/'; then
...
returns `decision: "allow"`, which tells Heretic to pre-approve the call:
...
case "$HERETIC_TOOL_INPUT_COMMAND" in
...
if [[ "$HERETIC_TOOL_INPUT_FILE_PATH" == *.go ]]; then
...
```jsonc
{ "command": "echo \"$(date -Iseconds) $HERETIC_TOOL_NAME\" >> ./tools.log" }
```
...
Heretic hooks are broadly compatible with [Claude Code
...
hooks run under Heretic unchanged. This document covers the Heretic-specific API
only — anything not documented here isn't guaranteed to work.

One intentional divergence: Heretic treats `updated_input` as a shallow-merge
...
4. `decision` precedence: `deny` > `allow` > `null`. First deny determines the
   ...
   is `allow`, Heretic pre-approves the tool call and skips the permission
