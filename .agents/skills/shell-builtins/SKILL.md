---
name: shell-builtins
description: Use when creating a new shell builtin command for Heretic (internal/shell/), editing an existing one, or when the user needs to understand how commands are intercepted in Heretic's embedded shell.
---

# Shell Builtins

Heretic's shell (`internal/shell/`) uses `mvdan.cc/sh/v3` for POSIX shell
