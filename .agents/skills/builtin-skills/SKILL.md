...
name: builtin-skills
description:
  Use when creating a new builtin skill for Heretic, editing an existing builtin
...
Heretic embeds skills directly into the binary via `internal/skills/builtin/`.
...
- `DiscoverBuiltin()` walks the embedded FS, parses each `SKILL.md`, and sets
  paths with the `heretic://skills/` prefix (e.g., `heretic://skills/jq/SKILL.md`).
- The View tool resolves `heretic://` paths from the embedded FS, not disk.
...
| -------------- | ----------------------- | ------------------------------------------ |
| `heretic-config` | `builtin/heretic-config/` | Heretic configuration help                   |
| `heretic-hooks`  | `builtin/heretic-hooks/`  | Authoring, configuring and debugging hooks |
